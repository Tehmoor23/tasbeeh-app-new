...
const NFC_CARDS_COLLECTION = 'nfc_cards';
...
const extractNfcUid = (tag) => {
  if (!tag || typeof tag !== 'object') return '';
  const candidate = tag.id || tag.uid || tag.identifier || tag.serialNumber || '';
  ...
  if (Array.isArray(tag?.ndefMessage) && tag.ndefMessage.length > 0) {
    ...
    if (first?.id) return String(first.id).trim().toUpperCase();
  }
  return '';
};
...
function AppContent() {
  ...
  const [nfcStatus, setNfcStatus] = useState('NFC wird initialisiert …');
  const [nfcRegistrationDraft, setNfcRegistrationDraft] = useState(null);
  ...
  const countAttendance = async (modeType, kind, locationName, selectedMember = null) => {
    ...
    // NFC-Erstregistrierung: vorhandene Auswahl-UI wiederverwenden, aber noch keine Anwesenheit schreiben.
    if (
      modeType === 'prayer'
      && kind === 'member'
      && nfcRegistrationDraft?.awaitingMemberSelection
      && selectedMember?.idNumber
    ) {
      setNfcRegistrationDraft((prev) => {
        if (!prev) return null;
        return {
          ...prev,
          member: {
            idNumber: String(selectedMember.idNumber),
            tanzeem: String(selectedMember.tanzeem || '').toLowerCase(),
            majlis: String(selectedMember.majlis || ''),
            name: String(selectedMember.name || ''),
          },
          awaitingMemberSelection: false,
          awaitingRescan: true,
        };
      });
      setToast('Mitglied ausgewählt. Bitte denselben NFC-Tag erneut scannen.');
  ...
  const lookupNfcCardMapping = useCallback(async (uid) => {
    if (!uid) return null;
    const card = await getDocData(NFC_CARDS_COLLECTION, uid);
    if (!card || card.active === false) return null;
    return card;
  }, []);

  const storeNfcCardMapping = useCallback(async (uid, member) => {
    if (!uid || !member?.idNumber) throw new Error('Ungültige NFC-Registrierungsdaten');
    const nowIso = new Date().toISOString();
    const existing = await getDocData(NFC_CARDS_COLLECTION, uid);
    await setDocData(NFC_CARDS_COLLECTION, uid, {
      memberId: String(member.idNumber),
  ...
  const handleNfcTagDiscovered = useCallback(async (tag) => {
    const uid = extractNfcUid(tag);
    if (!uid) {
      setToast('NFC-Tag konnte nicht gelesen werden');
      return;
    }

    if (!hasFirebaseConfig()) {
      setToast('Datenbankfehler – Firebase Konfiguration fehlt');
      return;
    }

    if (nfcRegistrationDraft?.awaitingRescan) {
      if (uid !== String(nfcRegistrationDraft?.uid || '').toUpperCase()) {
        setToast('Anderer NFC-Tag erkannt. Bitte denselben Tag erneut halten.');
        return;
      }
      if (!nfcRegistrationDraft?.member?.idNumber) {
        setToast('Bitte zuerst ein Mitglied auswählen.');
        return;
      }
      await storeNfcCardMapping(uid, nfcRegistrationDraft.member);
      setNfcRegistrationDraft(null);
      Vibration.vibrate(6);
      setToast('NFC erfolgreich registriert');
      return;
    }

    const mapping = await lookupNfcCardMapping(uid);
    if (!mapping?.memberId) {
      setNfcRegistrationDraft({
        uid,
        member: null,
        awaitingMemberSelection: true,
        awaitingRescan: false,
      });
      setToast('NFC noch nicht registriert. Bitte einmal registrieren.');
    ...
    const memberId = String(mapping.memberId || mapping.idNumber || '').trim();
    if (!memberId) {
      setToast('NFC-Zuordnung ungültig (memberId fehlt)');
      return;
    }

    const mappedMember = membersDirectory.find((entry) => String(entry?.idNumber || '') === memberId) || {
      idNumber: memberId,
      tanzeem: String(mapping.tanzeem || '').toLowerCase(),
      majlis: String(mapping.majlis || ''),
      name: String(mapping.name || ''),
    };

    await countAttendance('prayer', 'member', mappedMember.majlis || mapping.majlis || '', mappedMember);
  }, [
    countAttendance,
    lookupNfcCardMapping,
    membersDirectory,
    nfcRegistrationDraft,
    setQuickIdSearchVisible,
    storeNfcCardMapping,
  ]);

  useEffect(() => {
    nfcHandlerRef.current = handleNfcTagDiscovered;
  }, [handleNfcTagDiscovered]);

  useEffect(() => {
    if (Platform.OS === 'web') {
      setNfcStatus('NFC auf Web nicht unterstützt');
      return undefined;
    }
    ...
    const bootNfc = async () => {
      try {
        ...
        if (!supported) {
          if (mounted) setNfcStatus('NFC wird auf diesem Gerät nicht unterstützt');
          return;
        }

        await NfcManager.start();
        nfcSessionActiveRef.current = true;
        if (mounted) setNfcStatus('NFC bereit – Terminal hört dauerhaft auf Tags');

        NfcManager.setEventListener(NfcEvents.DiscoverTag, async (tag) => {
          if (nfcProcessingRef.current) return;
          nfcProcessingRef.current = true;
          try {
            await nfcHandlerRef.current?.(tag);
          } catch (error) {
            setToast('NFC-Verarbeitung fehlgeschlagen');
            console.error('NFC flow failed', error);
          } finally {
            try {
              await NfcManager.unregisterTagEvent();
              await NfcManager.registerTagEvent();
            } catch (error) {
              console.error('NFC register restart failed', error);
            }
        ...
        if (mounted) setNfcStatus('NFC konnte nicht initialisiert werden');
      }
    };
  ...
  const renderTerminal = () => {
    ...
    return (
      <ScrollView ref={terminalScrollRef} keyboardShouldPersistTaps="handled" contentContainerStyle={contentContainerStyle} showsVerticalScrollIndicator={false} stickyHeaderIndices={[0]}>
        ...
        <View style={[styles.quickSearchPanel, { borderColor: theme.border, backgroundColor: theme.card, marginTop: 10 }]}> 
          <Text style={[styles.noteText, { color: theme.text, textAlign: 'center' }]}>NFC Auto-Scan: {nfcStatus}</Text>
          {nfcRegistrationDraft?.awaitingMemberSelection ? (
            <Text style={[styles.noteText, { color: theme.muted, textAlign: 'center', marginTop: 4 }]}>NFC noch nicht registriert – bitte jetzt Mitglied auswählen.</Text>
          ) : null}
          {nfcRegistrationDraft?.awaitingRescan ? (
            <Text style={[styles.noteText, { color: theme.muted, textAlign: 'center', marginTop: 4 }]}>Bitte denselben NFC-Tag erneut an das Terminal halten, um die Registrierung abzuschließen.</Text>
          ) : null}
        </View>
